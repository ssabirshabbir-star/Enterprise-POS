...
function cleanItems(items = [], field = 'orderedQty') {
  ...
  for (const item of items) {
    ...
    if (field !== 'receivedQty' && (!Number.isInteger(productId) || productId <= 0))
      return { ok: false, message: 'Invalid product in item.' };
    if (quantity === null)
      return { ok: false, message: 'Item quantity must be greater than zero.' };
    if (field === 'orderedQty' && (cost === null || salePrice === null))
      return { ok: false, message: 'Invalid item price.' };
    clean.push({
      productId,
...
function cleanOrderPayload(payload = {}) {
  ...
  if (supplierId !== null && (!Number.isInteger(supplierId) || supplierId <= 0))
    return { ok: false, message: 'Invalid supplier.' };
  if (requisitionId !== null && (!Number.isInteger(requisitionId) || requisitionId <= 0))
    return { ok: false, message: 'Invalid requisition.' };
  ...
  return {
    ...
    payload: {
      ...
      status: ['DRAFT', 'PENDING_APPROVAL', 'APPROVED'].includes(
...
function cleanRequisitionPayload(payload = {}) {
  ...
  return {
    ...
    payload: {
      requestedDate:
        String(payload.requestedDate || '').trim() || new Date().toISOString().slice(0, 10),
...
async function updateRequisitionStatus(id, status, notes = '') {
  ...
  if (status === 'SUBMITTED' && current.status !== 'DRAFT')
    return { ok: false, message: 'Only draft requisitions can be submitted.' };
  if (['APPROVED', 'REJECTED'].includes(status) && current.status !== 'SUBMITTED')
    return { ok: false, message: 'Only submitted requisitions can be approved or rejected.' };
  const requisition = await poRepository.updateRequisitionStatus(
    requisitionId,
    status,
    access.profile.id,
    String(notes || '').trim()
  ...
  await activityRepository.createActivityLog({
    ...
    status: 'success',
    message: `Requisition ${status.toLowerCase()}`,
    metadata: { requisitionId },
  });
  return {
    ok: true,
    requisition,
    message: `Requisition ${status.replaceAll('_', ' ').toLowerCase()}.`,
  };
}
...
async function createOrder(payload = {}) {
  ...
  try {
    ...
    return {
      ok: true,
      order,
      message: 'Purchase order created for internal approval. Stock has not been updated.',
    };
  } catch (error) {
    if (error.message === 'REQUISITION_NOT_APPROVED')
      return { ok: false, message: 'Only approved requisitions can be converted to PO.' };
    if (error.message === 'REQUISITION_NOT_FOUND')
...
async function convertRequisitionToOrder(requisitionId, payload = {}) {
  ...
  if (!Number.isInteger(supplierId) || supplierId <= 0)
    return { ok: false, message: 'Supplier is required before converting to PO.' };
  try {
    const order = await poRepository.convertRequisitionToOrder(
    ...
    await activityRepository.createActivityLog({
      ...
      status: 'success',
      message: 'Requisition converted to PO',
      metadata: { requisitionId: Number(requisitionId), purchaseOrderId: order.id },
    });
    return { ok: true, order, message: 'Approved requisition converted to purchase order.' };
  } catch (error) {
    if (error.message === 'REQUISITION_NOT_APPROVED')
      return { ok: false, message: 'Only approved requisitions can be converted.' };
    if (error.message === 'REQUISITION_NOT_FOUND')
      return { ok: false, message: 'Requisition not found.' };
    throw error;
  }
...
async function sendToSupplier(id, notes = '') {
  const access = await requirePoAccess('approve');
  if (!access.ok) return access;
  const order = await poRepository.markSentToSupplier(
    Number(id),
    access.profile.id,
    String(notes || '').trim()
  );
  if (!order)
    return { ok: false, message: 'Only approved purchase orders can be sent to supplier.' };
  await activityRepository.createActivityLog({
    userId: access.profile.id,
    action: 'purchase_order.send_to_supplier',
    status: 'success',
    message: 'PO sent to supplier',
    metadata: { purchaseOrderId: Number(id) },
  });
...
async function confirmSupplier(id, payload = {}) {
  ...
  await activityRepository.createActivityLog({
    userId: access.profile.id,
    action: 'purchase_order.supplier_confirm',
    status: 'success',
...
async function receiveOrder(payload = {}) {
  ...
  for (const item of receivedItems.items) {
    if (!Number.isInteger(item.purchaseOrderItemId) || item.purchaseOrderItemId <= 0)
      return { ok: false, message: 'Invalid receiving item.' };
  }
  const subtotal = Number(
    receivedItems.items.reduce((sum, item) => sum + item.total, 0).toFixed(2)
  );
  const discount = money(payload.discount);
  const tax = money(payload.tax);
  if (discount === null || tax === null) return { ok: false, message: 'Invalid receiving totals.' };
  if (discount > subtotal)
    return { ok: false, message: 'Discount cannot exceed received subtotal.' };
  try {
    ...
    await activityRepository.createActivityLog({
      ...
      metadata: { purchaseOrderId, receiptId: result.receipt.id },
    });
    return {
      ok: true,
      ...result,
      message: 'Goods received and stock updated. Create the purchase invoice when ready.',
    };
  } catch (error) {
    if (error.message === 'PO_NOT_FOUND')
      return { ok: false, message: 'Purchase order not found.' };
    if (error.message === 'PO_NOT_RECEIVABLE')
      return {
        ok: false,
        message: 'This purchase order cannot be received in its current status.',
      };
    if (error.message === 'RECEIVE_EXCEEDS_ORDERED')
      return { ok: false, message: 'Received quantity cannot exceed ordered quantity.' };
    if (error.code === '23505')
      return { ok: false, message: 'Goods receipt number already exists. Please try again.' };
    throw error;
  }
}
...
async function createInvoiceFromReceipt(payload = {}) {
  ...
  if (!Number.isInteger(goodsReceiptId) || goodsReceiptId <= 0)
    return { ok: false, message: 'Goods receipt is required.' };
  const discount = money(payload.discount);
  const tax = money(payload.tax);
  const paidAmount = money(payload.paidAmount);
  if (discount === null || tax === null || paidAmount === null)
    return { ok: false, message: 'Invalid invoice totals.' };
  try {
    const invoice = await poRepository.createInvoiceFromReceipt(
      {
        ...
        invoiceDate:
          String(payload.invoiceDate || '').trim() || new Date().toISOString().slice(0, 10),
        discount,
        tax,
        paidAmount,
      },
    ...
    await activityRepository.createActivityLog({
      ...
      status: 'success',
      message: 'Purchase invoice created from goods receipt',
      metadata: invoice,
    });
    ...
    if (error.message === 'RECEIPT_ALREADY_INVOICED')
      return { ok: false, message: 'This goods receipt already has an invoice.' };
    if (error.message === 'INVALID_INVOICE_TOTALS')
      return { ok: false, message: 'Discount or paid amount is invalid.' };
    if (error.code === '23505')
      return { ok: false, message: 'Purchase invoice number already exists.' };
    throw error;
  }
}
...
async function listReceipts(purchaseOrderId) {
  ...
  return {
    ok: true,
    receipts: await poRepository.listReceipts(purchaseOrderId ? Number(purchaseOrderId) : null),
  };
}
