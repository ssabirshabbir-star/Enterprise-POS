...
function registerAccessControlRoutes(ipcMain) {
  ipcMain.handle('/users/list', async (_event, filters) => {
    try { return await accessService.listUsers(filters || {}); } catch (error) { return safeError(error, 'Users list error:'); }
  });
  ipcMain.handle('/users/create', async (_event, payload) => {
    try { return await accessService.createUser(payload || {}); } catch (error) { return safeError(error, 'Users create error:'); }
  });
  ipcMain.handle('/users/update', async (_event, { id, payload }) => {
    try { return await accessService.updateUser(id, payload || {}); } catch (error) { return safeError(error, 'Users update error:'); }
  });
  ipcMain.handle('/users/status', async (_event, { id, isActive }) => {
    try { return await accessService.setUserActive(id, isActive); } catch (error) { return safeError(error, 'Users status error:'); }
  });
  ...
  ipcMain.handle('/roles/create', async (_event, payload) => {
    try { return await accessService.createRole(payload || {}); } catch (error) { return safeError(error, 'Roles create error:'); }
  });
  ipcMain.handle('/roles/update', async (_event, { id, payload }) => {
    try { return await accessService.updateRole(id, payload || {}); } catch (error) { return safeError(error, 'Roles update error:'); }
  });
  ipcMain.handle('/roles/permissions', async (_event, roleId) => {
    try { return await accessService.permissionsByRole(roleId); } catch (error) { return safeError(error, 'Role permissions error:'); }
  });
  ipcMain.handle('/roles/permissions/save', async (_event, { roleId, permissionIds }) => {
    try { return await accessService.assignPermissions(roleId, permissionIds || []); } catch (error) { return safeError(error, 'Role permissions save error:'); }
  });
}
