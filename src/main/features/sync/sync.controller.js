...
function registerSyncRoutes(ipcMain) {
  ipcMain.handle('/sync/status', async () => {
    try {
      ...
      return safeError(error, 'Sync status error:');
    }
  });
  ipcMain.handle('/sync/queue', async () => {
    try {
      return await syncService.listQueue();
    } catch (error) {
      return safeError(error, 'Sync queue error:');
    }
  });
  ipcMain.handle('/sync/run', async () => {
    try {
      ...
      return safeError(error, 'Sync run error:');
    }
  });
  ipcMain.handle('/sync/retry-failed', async () => {
    try {
      ...
      return safeError(error, 'Sync retry error:');
    }
  });
}
