...
(function BillingRendererModule() {
  ...
  function onKeyDown(e) {
    ...
    if (e.key === 'F4')  { e.preventDefault(); A().completeSale();              return; }
    if (e.key === 'F6')  { e.preventDefault(); C().setPaymentMethod('Cash');    return; }
    if (e.key === 'F7')  { e.preventDefault(); C().setPaymentMethod('Card');    return; }
    if (e.key === 'F8')  {
      e.preventDefault();
    ...
    if (e.key === 'F9')  { e.preventDefault(); C().setPaymentMethod('Mixed');   return; }
    if (e.key === 'F10') { e.preventDefault(); A().printReceipt();              return; }
    if (e.key === 'F11') { e.preventDefault(); A().downloadPdf();               return; }
    if (e.key === 'F12') { e.preventDefault(); A().reprintLastBill();           return; }

    // Ctrl shortcuts
    if (e.ctrlKey && !e.shiftKey) {
      const k = e.key.toLowerCase();
      if (k === 'b')      { e.preventDefault(); C().setPaymentMethod('Bank');       return; }
      if (k === 'w')      { e.preventDefault(); A().sendWhatsApp();                 return; }
      if (k === 's')      { e.preventDefault(); A().holdSale();                     return; }
      if (k === 'r')      { e.preventDefault(); A().validateSaleAction('refund');   return; }
      if (k === 'e')      { e.preventDefault(); A().validateSaleAction('exchange'); return; }
      if (e.key === 'Delete') { e.preventDefault(); A().clearCartConfirm();         return; }
    }
  ...
  function attachEvents() {
    ...
    if (si) {
      si.addEventListener('input', e => {
        ...
        if (!v) { C().clearSearchResults(); return; }
      ...
      si.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape') { e.target.value = ''; C().clearSearchResults(); }
        if (e.key === 'ArrowDown') {
          const first = document.getElementById('posSearchResults')
            ?.querySelector('[data-product-id]');
          if (first) { e.preventDefault(); first.focus(); }
        }
      });
      document.getElementById('posSearchFocusButton')
        ?.addEventListener('click', () => si.focus());
    }
    ...
    document.getElementById('posSearchResults')?.addEventListener('click', e => {
      ...
      if (product) { C().addToCart(product); if (si) si.value = ''; }
    });
    ...
    if (tbody) {
      tbody.addEventListener('change', e => {
        const el = e.target;
        if (el.dataset.cartQty   !== undefined) C().refreshCartItemDisplay(Number(el.dataset.cartQty),   'qty',   el.value);
        if (el.dataset.cartPrice !== undefined) C().refreshCartItemDisplay(Number(el.dataset.cartPrice), 'price', el.value);
        if (el.dataset.cartDisc  !== undefined) C().refreshCartItemDisplay(Number(el.dataset.cartDisc),  'disc',  el.value);
      });
      tbody.addEventListener('click', e => {
        const btn = e.target.closest('[data-remove-item]');
        if (btn) { C().removeCartItem(Number(btn.dataset.removeItem)); return; }
        ...
        if (dec) {
          ...
          if (newQty <= 0) C().removeCartItem(idx); // remove when reaching zero
    ...
    document.getElementById('posDiscountType')
      ?.addEventListener('change', () => C().updateDisplayTotals());
    ...
    document.getElementById('holdSaleButton')
      ?.addEventListener('click',  () => A().holdSale());
    document.getElementById('thermalPrintButton')
      ?.addEventListener('click',  () => A().printReceipt());
    document.getElementById('posDownloadPdfButton')
      ?.addEventListener('click',  () => A().downloadPdf());
    document.getElementById('reprintLastBillButton')
      ?.addEventListener('click',  () => A().reprintLastBill());
    document.getElementById('clearCartButton')
      ?.addEventListener('click',  () => A().clearCartConfirm());
    document.getElementById('refundSaleButton')
      ?.addEventListener('click',  () => A().validateSaleAction('refund'));
    document.getElementById('exchangeSaleButton')
      ?.addEventListener('click',  () => A().validateSaleAction('exchange'));
    document.getElementById('posWhatsappButton')
    ...
    document.getElementById('heldSalesList')?.addEventListener('click', e => {
      ...
      if (r) { A().restoreHold(r.dataset.restoreHold); return; }
    ...
    // ── Billing mode ─────────────────────────────────────────────────────────
    document.getElementById('retailModeButton')
      ?.addEventListener('click', () => C().setBillingMode('retail'));
    document.getElementById('wholesaleModeButton')
    ...
    // ── Customer modal ────────────────────────────────────────────────────────
    document.getElementById('posCustomerAddButton')
      ?.addEventListener('click', () => C().openCustomerModal());
    document.getElementById('quickCustomerFocusButton')
      ?.addEventListener('click', () => C().openCustomerModal());
    document.getElementById('posCustomerModalForm')
      ?.addEventListener('submit', e => A().saveCustomer(e));
    document.querySelectorAll('[data-pos-customer-close]').forEach(el =>
      el.addEventListener('click', () => C().closeCustomerModal())
    );
