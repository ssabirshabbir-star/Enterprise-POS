...
(function BillingCartModule() {
  ...
  function setEl(id, v) { const e = $id(id); if (e) e.textContent = v; }
  ...
  function showMsg(text, isError) {
    ...
    el.style.cssText = isError
      ...
      : 'display:block;background:#f0fdf4;color:#166534;border:1px solid #86efac';
  ...
  function getCart()          { return carts[activeCart]; }
  function getActiveCart()    { return activeCart; }
  function getCustomers()     { return customers; }
  function setCustomers(arr)  { customers = Array.isArray(arr) ? arr : []; }
  function getLastReceipt()   { return lastReceipt; }
  function setLastReceipt(r)  { lastReceipt = r; }
  ...
  function addToCart(product) {
    const price = billingMode === 'wholesale' && Number(product.wholesalePrice) > 0
    ...
    if (existing) {
      ...
      existing.displayTotal = Math.max((existing.quantity * existing.unitPrice) - existing.discount, 0);
    } else {
      getCart().items.push({
  ...
  function switchToCart(index) {
    ...
    ['cartDiscount', 'paidAmount'].forEach(id => { const e = $id(id); if (e) e.value = '0'; });
  }
  ...
  function updateDisplayTotals() {
    ...
    const displayPaid    = parseFloat($id('paidAmount')?.value    || '0') || 0;
    ...
    if (payBtn) payBtn.textContent = `Rs. ${displayGrand.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  }
  ...
  function setPaymentMethod(method) {
    ...
    document.querySelectorAll('[data-payment-set]').forEach(btn => {
      ...
      btn.style.outline   = on ? '2.5px solid rgba(255,255,255,.7)' : '';
      btn.style.transform = on ? 'scale(1.03)' : '';
    });
  }
...
Method: ${receipt.paymentMethod || 'Cash'}${coupons.length
...
────────────────────────</div>`;
  ...
  function renderHeldSalesList(holds) {
    ...
    el._holds = holds;
    if (!holds.length) {
      el.innerHTML = '<p style="padding:6px 10px;color:#9ca3af;font-size:.75rem">No held sales.</p>';
      return;
    }
  ...
  function clearSearchResults() {
    ...
    if (el) { el.innerHTML = ''; el._products = null; }
  }
  ...
  window.BillingCart = {
    // State accessors
    getCart, getActiveCart, getCustomers, setCustomers,
    getLastReceipt, setLastReceipt, getBillingMode,
    // Cart mutations
    addToCart, removeCartItem, clearCartDisplay,
    switchToCart, refreshCartItemDisplay,
    setCartCustomer,                      // controlled customer ID setter
    // Rendering
    renderCart, updateDisplayTotals,
    setBillingMode, setPaymentMethod,
    // Customer display
    renderCustomerSelect, updateCustomerBalanceDisplay,
    loadCustomerIntoCart, openCustomerModal, closeCustomerModal,
    // Receipt + holds display
    renderReceiptPreview, renderHeldSalesList, restoreHeldItemsToCart,
    clearSearchResults,
    // Payload + utilities
    getCartPayload, showMsg, showModalMsg, esc, fmt
  };
})();
