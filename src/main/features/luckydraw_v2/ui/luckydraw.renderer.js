...
(function LuckyDrawV2RendererModule() {
  ...
  function showPanel(panelId) {
    ['ldv2CampaignsPanel', 'ldv2ParticipantsPanel', 'ldv2DrawPanel', 'ldv2WinnersPanel'].forEach(
      (id) => {
        const el = $id(id);
  ...
  function openCampaignForm(campaign = null) {
    ...
    $id('ldv2CampaignMinPurchase').value = campaign?.minimumPurchase || '1';
  ...
  function updateDrawRemaining(campaign, newWinnersCount) {
    ...
    const remaining = Math.max(
      0,
      (campaign.totalWinners || 0) - (campaign.totalWinnersDrawn || 0) - (newWinnersCount || 0)
    );
    const remEl = $id('ldv2DrawRemaining');
    if (remEl) {
      remEl.textContent = `Remaining winner slots: ${remaining}`;
      remEl.style.display = '';
    }
  }
  ...
  function attachEvents() {
    ...
    $id('ldv2CampaignTbody')?.addEventListener('click', (e) => {
      ...
      if (action === 'edit') {
        ...
        const cData = (() => {
          try {
            return JSON.parse(btn.dataset.campaign || '{}');
          } catch (_) {
            return {};
          }
        })();
    ...
    $id('ldv2PartSearchInput')?.addEventListener('input', (e) => {
      ...
      window.__ldv2SearchTimer = setTimeout(
        () =>
          A().loadParticipants({ campaignId: _activeCampaignId, search: e.target.value.trim() }),
        350
      );
    });
    ...
    $id('ldv2RunDrawBtn')?.addEventListener('click', () => {
      ...
      if (!campaignId) {
        showMsg('Select a campaign first.', true);
        return;
      }
