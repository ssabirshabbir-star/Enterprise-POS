...
async function findInvoice(filters = {}) {
  ...
  if (customer) {
    ...
    clauses.push(`(LOWER(customers.name) LIKE $${params.length} OR COALESCE(customers.phone, '') LIKE $${params.length})`);
  }
  if (clauses.length === 0) return null;
  const result = await getPool().query(
    `
      SELECT DISTINCT sales.*, customers.name AS customer_name, users.full_name AS cashier_name
      FROM sales
  ...
  return {
    ...
    items: items.rows.map((row) => ({
      ...
      returnableQuantity: Math.max(Number(row.quantity) - Number(row.returned_quantity), 0),
...
async function createReturn(payload, userId) {
  return withTransaction(async (client) => {
    ...
    const saleResult = await client.query('SELECT * FROM sales WHERE id = $1 FOR UPDATE', [payload.saleId]);
    ...
    const returnResult = await client.query(
      `
        ...
        RETURNING *
      `,
      [payload.returnNumber, sale.id, sale.customer_id, terminal.id, userId, payload.refundMethod, payload.reason || null, payload.totalRefund]
    );
    ...
    for (const item of payload.items) {
      ...
      const returnedResult = await client.query('SELECT COALESCE(SUM(quantity), 0)::numeric AS quantity FROM return_items WHERE sale_item_id = $1', [item.saleItemId]);
      const alreadyReturned = Number(returnedResult.rows[0].quantity || 0);
      if (alreadyReturned + Number(item.quantity) > Number(saleItem.quantity)) throw new Error('RETURN_QTY_EXCEEDED');

      const unitRefund = Number(saleItem.quantity) > 0 ? Number(saleItem.total) / Number(saleItem.quantity) : Number(saleItem.unit_price);
      ...
      await client.query(
        ...
        [returnRecord.id, item.saleItemId, saleItem.product_id, item.quantity, unitRefund, lineTotal]
      );

      const previousStock = Number(saleItem.current_stock);
      const newStock = previousStock + Number(item.quantity);
      await client.query('UPDATE products SET current_stock = $2, updated_at = NOW() WHERE id = $1', [saleItem.product_id, newStock]);
      if (warehouseId) {
        await client.query(
          `
      ...
      await client.query(
        ...
        `,
        [saleItem.product_id, warehouseId, item.quantity, previousStock, newStock, returnRecord.id, payload.reason || 'Sale return', returnRecord.return_number, userId]
      );
    }

    await client.query('INSERT INTO refund_payments (return_id, refund_method, amount) VALUES ($1, $2, $3)', [returnRecord.id, payload.refundMethod, payload.totalRefund]);

    if (payload.refundMethod === 'Customer Credit' && sale.customer_id) {
      const balanceResult = await client.query('SELECT current_balance FROM customers WHERE id = $1 FOR UPDATE', [sale.customer_id]);
      ...
      await client.query('UPDATE customers SET current_balance = $2, updated_at = NOW() WHERE id = $1', [sale.customer_id, newBalance]);
      await client.query(
        'INSERT INTO customer_ledger (customer_id, sale_id, return_id, entry_type, debit, credit, balance, notes) VALUES ($1, $2, $3, $4, 0, $5, $6, $7)',
        [sale.customer_id, sale.id, returnRecord.id, 'REFUND_CREDIT', payload.totalRefund, newBalance, returnRecord.return_number]
      );
    }
    ...
    await syncRepository.queueOperation({
      ...
      payload: { returnNumber: returnRecord.return_number, saleId: sale.id, totalRefund: payload.totalRefund }
    });
    return returnRecord;
  });
}
