...
function generateInvoiceNumber() {
  ...
  const terminal = String(process.env.POS_TERMINAL_CODE || require('os').hostname() || 'T1')
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, '')
    .slice(0, 8) || 'T1';
...
async function completeSale(payload = {}) {
  ...
  for (const item of items) {
    ...
    if (!Number.isInteger(productId) || productId <= 0) return { ok: false, message: 'Invalid cart product.' };
    if (itemQuantity === null) return { ok: false, message: 'Cart quantity must be greater than zero.' };
    if (unitPrice === null || discount === null) return { ok: false, message: 'Invalid cart price or discount.' };
    ...
    cleanItems.push({ productId, quantity: itemQuantity, unitPrice, discount, total: Number((gross - discount).toFixed(2)) });
  }
  ...
  if (discount === null || tax === null || paidAmount === null) return { ok: false, message: 'Invalid billing totals.' };
  ...
  if (!['Cash', 'Card', 'Bank', 'Credit', 'Mixed'].includes(paymentMethod)) return { ok: false, message: 'Payment method is invalid.' };
  if (paidAmount < grandTotal && paymentMethod !== 'Credit') return { ok: false, message: 'Paid amount cannot be less than grand total unless payment method is Credit.' };
  if (paymentMethod === 'Credit' && !payload.customerId) return { ok: false, message: 'Credit sale requires a customer.' };
  ...
  try {
    ...
    if (error.message.includes('Insufficient stock')) return { ok: false, message: 'Insufficient stock for one or more products.' };
    if (error.message.includes('inactive')) return { ok: false, message: 'One or more products are inactive.' };
    if (error.message === 'CREDIT_SALE_REQUIRES_CUSTOMER') return { ok: false, message: 'Credit sale requires a real customer, not Walk-in Customer.' };
    if (error.message === 'CREDIT_LIMIT_EXCEEDED') return { ok: false, message: 'Customer credit limit exceeded.' };
    if (error.code === '23505') return { ok: false, message: 'Duplicate invoice number. Please try again.' };
    throw error;
  }
}
