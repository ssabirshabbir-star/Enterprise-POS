...
(function CustomersRendererModule() {
  ...
  function getSelectedCustomer()      { return _selectedCustomer; }
  function setSelectedCustomer(c)     { _selectedCustomer = c; }
  ...
  function renderListStats(customers) {
    ...
    set('customerTotalSales', `PKR ${totalSales.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
  }
  function applyTabFilter(customers, tab) {
    ...
    const DAY = 86400000;
    switch (tab) {
      case 'active':   return customers.filter(c => c.isActive);
      case 'inactive': return customers.filter(c => !c.isActive);
      case 'vip':      return customers.filter(c => c.group === 'vip');
      case 'recent':   return customers.filter(c => {
        const d = c.stats?.lastPurchaseDate;
        return d && (now - new Date(d).getTime()) < 30 * DAY;
      });
      case 'regular':  return customers.filter(c => c.group === 'regular');
      case 'credit':   return customers.filter(c => Number(c.currentBalance || 0) > 0);
  ...
  function highlightSelectedRow(customerId) {
    document.querySelectorAll('#customerPageList tr').forEach(tr =>
  ...
  function renderCustomerDetails(data) {
    ...
    panel.innerHTML = `
      ...
      <div style="padding:10px 16px">
        ...
        ${sales.length ? `<div style="max-height:220px;overflow-y:auto">
          <table style="width:100%;border-collapse:collapse;font-size:.71rem">
            ...
            <tbody>${sales.map(s => `<tr style="border-top:1px solid #f3f4f6">
  ...
  function openEditorModal(customer) {
    ...
    if (customer) {
      ...
      const idEl = $id('customerFormId'); if (idEl) idEl.value = '';
    }
  ...
  async function promptPayment(customerId) {
    ...
    } catch (_) { amount = window.prompt('Enter payment amount (PKR):') || ''; }
    ...
    let note = '';
    try { note = await window.posApi.dialog.prompt('Payment note (optional):', ''); } catch (_) {}

    A().postPayment(customerId, { amount: parsed, note: (note || '').trim() || undefined });
  }
  ...
  function attachEvents() {
    ...
    // ── Tab buttons ───────────────────────────────────────────────────────────
    document.querySelectorAll('[data-customer-tab]').forEach(btn =>
      btn.addEventListener('click', () => setActiveTab(btn.dataset.customerTab))
    );
    ...
    $id('resetCustomerButton')?.addEventListener('click', () => {
      $id('customerForm')?.reset();
      const idEl = $id('customerFormId'); if (idEl) idEl.value = '';
      const msgEl = $id('customerFormMsg'); if (msgEl) msgEl.textContent = '';
      $id('customerEditorTitle') && ($id('customerEditorTitle').textContent = 'Add Customer');
    });

    // ── Close editor modal ────────────────────────────────────────────────────
    document.querySelectorAll('[data-customer-modal-close]').forEach(el =>
      el.addEventListener('click', () => closeEditorModal())
    );
    ...
    $id('customerPageList')?.addEventListener('click', e => {
      ...
      if (viewBtn) {
        ...
        if (cust) { setSelectedCustomer(cust); A().loadCustomerDetails(Number(cust.id)); }
        return;
      }
      ...
      if (row) {
        ...
        if (cust) { setSelectedCustomer(cust); highlightSelectedRow(cust.id); }
      }
    });
    ...
    $id('customerDetailsPanel')?.addEventListener('click', e => {
      ...
      if (pay) { promptPayment(Number(pay.dataset.payCustomer)); return; }
    ...
    document.querySelectorAll('#customerDeleteInactiveButton').forEach(btn =>
      btn.addEventListener('click', () => A().deleteInactiveCustomers())
    );
    ...
    document.querySelectorAll('#openCustomerGroupsButton').forEach(btn =>
      btn.addEventListener('click', () => {
        const modal = $id('customerGroupsModal');
    ...
    document.querySelectorAll('#customerWhatsAppButton').forEach(btn =>
      btn.addEventListener('click', () => {
        if (_selectedCustomer) { openWhatsAppModal(_selectedCustomer); }
    ...
    document.querySelectorAll('[data-customer-whatsapp-close]').forEach(el =>
    ...
    document.querySelectorAll('[data-page-tool="customers"]').forEach(btn =>
    ...
    document.querySelectorAll('#customerBulkActionsButton').forEach(btn =>
      btn.addEventListener('click', () => {
        if (!_selectedIds.size) { showMsg('Select at least one customer for bulk actions.', true); return; }
        showMsg(`${_selectedIds.size} customer${_selectedIds.size !== 1 ? 's' : ''} selected. Bulk actions coming soon.`);
      })
    );
  }
  ...
  window.CustomersRenderer = {
    ...
    // Modal management
    openEditorModal, closeEditorModal,
    openWhatsAppModal, closeWhatsAppModal,
    // Feedback
    showMsg, showFormMsg,
    // Utilities
    esc, fmt
