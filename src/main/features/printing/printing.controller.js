...
function registerPrintingRoutes(ipcMain) {
  ipcMain.handle('/printing/printers', async () => {
    try {
      ...
      return safeError(error, 'Printer list error:');
    }
  });
  ipcMain.handle('/printing/settings/get', async () => {
    try {
      ...
      return safeError(error, 'Printer settings get error:');
    }
  });
  ipcMain.handle('/printing/settings/save', async (_event, settings) => {
    try {
      ...
      return {
        ok: true,
        settings: await printingService.savePrinterSettings(settings || {}),
        message: 'Printer settings saved.',
      };
    } catch (error) {
      return safeError(error, 'Printer settings save error:');
    }
  });
  ipcMain.handle('/printing/receipt/preview', async (_event, receipt) => {
    try {
      ...
      return {
        ok: true,
        html: printingService.buildReceiptHtml(receipt, settings),
        escpos: printingService.buildEscPosReceipt(receipt, settings),
      };
    } catch (error) {
      return safeError(error, 'Receipt preview error:');
    }
  });
    ...
    try {
      ...
      return {
        ...
        message: result.success
          ...
          : result.failureReason || 'Print failed.',
      };
    } catch (error) {
      return safeError(error, 'Receipt print error:');
    }
  });
  ipcMain.handle('/printing/receipt/pdf', async (event, { receipt, options }) => {
    try {
      ...
      if (!receipt?.invoiceNumber)
        return { ok: false, message: 'Complete a sale before downloading an invoice PDF.' };
      ...
      if (saveResult.canceled || !saveResult.filePath)
        return { ok: false, canceled: true, message: 'PDF download cancelled.' };
      const result = await printingService.exportReceiptPdf(
        receipt,
        saveResult.filePath,
        options || {}
      );
      ...
      return safeError(error, 'Receipt PDF export error:');
    }
  });
}
