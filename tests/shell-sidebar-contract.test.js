...
test('sidebar toggle uses the shared three-line menu button treatment', () => {
  ...
  assert.match(
    html,
    /id="sidebarToggle"[\s\S]*<span class="epos-sidebar-toggle-icon" aria-hidden="true">☰<\/span>/
  );
  assert.match(
    css,
    /\.epos-sidebar-toggle\s*{[\s\S]*width:\s*38px;[\s\S]*height:\s*38px;[\s\S]*border:\s*0;[\s\S]*border-radius:\s*12px;[\s\S]*background:\s*#ffffff;[\s\S]*color:\s*#2563eb;[\s\S]*font-size:\s*24px;[\s\S]*font-weight:\s*950;[\s\S]*inset 2px 2px 5px/
  );
  assert.match(
    css,
    /\.epos-sidebar-toggle-icon\s*{[\s\S]*display:\s*block;[\s\S]*line-height:\s*1;/
  );
  assert.doesNotMatch(css, /\.epos-sidebar-toggle span\s*{[\s\S]*height:\s*2px;/);
